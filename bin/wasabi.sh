...
# FIXME: don't rebuild, cp dist/* target/*
  ...
  (cd modules/ui; \
    mkdir -p target; \
    for f in build/constants.json app node_modules bower.json Gruntfile.js default_constants.json karma.conf.js karma-e2e.conf.js package.json test .bowerrc; do \
      cp -r ${f} target; \
    done; \
