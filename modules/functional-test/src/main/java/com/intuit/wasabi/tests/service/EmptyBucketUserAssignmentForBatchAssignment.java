 ...
 * This test class tests scenario where a user is assigned to a batch of experiments
 * and the assigned buckets are deleted to verify that when the assignment call is made
 * to the same user, then the user should be assigned to one of the available buckets 
 * in each of the experiments... to keep things simple creating two experiments with
 * two buckets for each experiment 
 *
 */
public class EmptyBucketUserAssignmentForBatchAssignment extends TestBase {
    ...
    List<Experiment> testExperimentsList = null; // holds experiments that are created in the test class to be deleted eventually in the tearDown

    List<String> assignedBucketLabels = null; // holds the labels of the buckets to which the test user has been assigned

    @BeforeClass
    public void setUp()
    {
        //lets create application and assign it to the variable
        batchAssignmentEmptyBucketTestApp = ApplicationFactory.createApplication().setName("batchAssignmentEmptyBucketTestApp");
    ...
    /**
     * This test case tests scenario where we do batch assignment of a user to
     * list of experiments, delete the assigned buckets and do assignment again
     * for the same user against the same experiments should make a new assignment
     */
    @Test
    public void createBatchAssignmentAndDeleteAssignedBucketTest() {
        ...
        for(int i = 1 ; i <= 2 ; i++) {
            Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),-1).setApplication(batchAssignmentEmptyBucketTestApp);
        ...
        List<Assignment> assignmentsList = postAssignments(batchAssignmentEmptyBucketTestApp, new User("user1234"), expList, "PROD");
        ...
        // do a POST of assignments for same user with same context- This should be new Assignment as previous assignment got deleted
    ...
    /**
     * This test case tests scenario where we do batch assignment of a user to
     * list of experiments, delete all the buckets and when we do assignment again
     * for the same user against the same experiments should get no open bucket
     */
    @Test
    public void createBatchAssignmentAndDeleteAllBucketTest() {
        ...
        Map<Integer,List<Bucket>> experimentBucketsMap = new HashMap<Integer,List<Bucket>>(); // map holds all the buckets within an experiment
    ...
    /**
     * The below method tears down the test experiments that are created
     * that we stored using the testExperimentsList
     */
    @AfterClass()
