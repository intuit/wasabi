...
public class RuleTest extends TestBase {
    ...
    public void validSegmentationInput(String input) {
        LOGGER.debug(input);
    ...
    @Test(groups = {"stage1"}, dependsOnMethods = "validSegmentationInput")
    public void updateBadRuleToNoRuleExperiment() {
    ...
    @Test(groups = {"stage1"}, dependsOnMethods = "validSegmentationInput")
    public void updateBadRuleToExistingRuleExperiment() {
    ...
    public void createExperimentWithInvalidRule(String input) {
        response = apiServerConnector.doPost("/experiments", input);
        assertReturnCode(response, HttpStatus.SC_BAD_REQUEST);
    }
    ...
    public void changingStates(String state) {
        for (Experiment experiment : validExperimentsMaps.values()) {
    ...
    @Test(dependsOnMethods = {"changingStates"}, groups = {"changingStates"})
    ...
    @Test(dependsOnGroups = {"changingStates"}, groups = {"assignment"})
    public void validRuleAssignment() {
        for (Experiment experiment : validExperimentsMaps.values()) {
            if (experiment.applicationName.contains("valid")) {
                String baseUrl = String.format("/assignments/applications/%s/experiments/%s/users/",
                        experiment.applicationName, experiment.label);
    ...
    public void setupExperimentForAssignmentWithoutProfile() {
    ...
    @Test(dependsOnMethods = {"setupExperimentForAssignmentWithoutProfile"}, groups = ("assignmentWithOutProfile"))
    ...
    @AfterClass
    public void cleanUp() {
        toCleanUp.addAll(validExperimentsMaps.values());
        cleanUpExperiments();
    }
