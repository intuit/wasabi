...
public class IntegrationMutualExclusion extends TestBase {
	...
	private Experiment expiredExperiment;
	private Experiment mutualExclusiveExperiment;
	List<Experiment> excludedExperimentList= new ArrayList<Experiment>();
	...
	/**
	 * The below test case tests a scenario where we try to do
	 * mutual exclusion with expired experiment
	 * Written for https://github.com/intuit/wasabi/issues/106
	 */
	@Test
	public void testMutualExlusionForExpiredExperiment()
	{
		//create an experiment
		experiment = ExperimentFactory.createExperiment();
		expiredExperiment = ExperimentFactory.createExperiment();

		//set start time and end time in previous for expired Experiment
		expiredExperiment.startTime = TestUtils.relativeTimeString(-10);
		expiredExperiment.endTime = TestUtils.relativeTimeString(-5);
		Experiment expired = postExperiment(expiredExperiment);
		Experiment exp = postExperiment(experiment);
		expiredExperiment.update(expired);
		experiment.update(exp);

		
		excludedExperimentList.add(expiredExperiment);
		Response resp = postExclusions(experiment, excludedExperimentList);


		//TO-DO check with calvin/ilya and zero in on the response code
		Assert.assertEquals(resp.getStatusCode(), HttpStatus.SC_CREATED);
		String status = JsonPath.with(resp.asString()).get("exclusions[0].status");
		Assert.assertEquals(status, "FAILED");
	}
	
	/**
	 * the below test case tests the scenario where we try to do 
	 * mutual exclusion with two experiments one being expired
	 * and other being Non expired
	 * Written for https://github.com/intuit/wasabi/issues/106
	 */
	@Test(dependsOnMethods = {"testMutualExlusionForExpiredExperiment"})
	public void testMutualExclusionForExpiredAndNonExpiredExperiment()
	{
		
		//creating and adding the non-expired experiment to list of excluding experiment
		mutualExclusiveExperiment = ExperimentFactory.createExperiment();
		Experiment mutualExclusiveExp = postExperiment(mutualExclusiveExperiment);
		mutualExclusiveExperiment.update(mutualExclusiveExp);
		excludedExperimentList.add(mutualExclusiveExperiment);
		...
		//TO-DO check with calvin/ilya and zero in on the response code
		Assert.assertEquals(resp.getStatusCode(),HttpStatus.SC_CREATED);
		String status1 = JsonPath.with(resp.asString()).get("exclusions[0].status");
		String status2 = JsonPath.with(resp.asString()).get("exclusions[1].status");
		Assert.assertEquals(status1, "FAILED");
	...
	public void cleanup() {
		...
		toCleanUp.add(expiredExperiment);
		toCleanUp.add(mutualExclusiveExperiment);
