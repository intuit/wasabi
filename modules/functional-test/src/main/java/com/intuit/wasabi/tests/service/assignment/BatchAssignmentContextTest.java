 ...
 * considers  batch-assignment calls with different contexts 
...
public class BatchAssignmentContextTest extends TestBase{
    ...
    public void createAssignmentForExperimentStartedInPast() {
        ...
        for(int i = 1 ; i <= numberOfExperimentsPerBatch ; i++) {
            Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),-1).setApplication(batchAssignmentContextTestApp);
        ...
        List<Assignment> assignmentsList = postAssignments(batchAssignmentContextTestApp, new User("user1234"), expList, "PROD");
        ...
        // do a POST of assignments for user1234 with context set as DEV and this should all be NEW ASSIGNMENT as it is a different context
    ...
    public void createAssignmentForExperimentStartAndEndInPast() {
        ...
        for(int i = 1 ; i <= numberOfExperimentsPerBatch ; i++) {
            Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),-5,-1).setApplication(batchAssignmentContextTestApp);
            exp = postExperiment(exp);
        ...
        List<Assignment> assignmentsList = postAssignments(batchAssignmentContextTestApp, new User("user5678"), expList, "PROD");
    ...
    public void createAssignmentForExperimentStartAndEndInFuture() {
        ...
        for(int i = 1 ; i <= numberOfExperimentsPerBatch ; i++) {
            Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),1,5).setApplication(batchAssignmentContextTestApp);
            exp = postExperiment(exp);
