 ...
 * This test class tests scenario where the wasabi application
 * considers a single-assignment calls with different contexts 
 * as two different assignments 
 */
public class SingleAssignmentContextTest extends TestBase{
    ...
    public void setUp()
    {
        ...
        singleAssignmentContextTestApp = ApplicationFactory.createApplication().setName("singleAssignmentContextTestApp");
    ...
    /**
     * This is the actual method that tests
     * for SUT
     */
    @Test
    public void createAssignmentForExperimentStartedInPast()
    {
        ...
        Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),-1).setApplication(singleAssignmentContextTestApp);
        ...
        //lets clear the cache and do an assignment for a user with context set to PROD so that we will get back existing assignment
        clearAssignmentsMetadataCache();
    ...
    /**
     * This is the  method that tests assignments for an experiment that has 
     * both start and end date in the past
     */
    @Test
    public void createAssignmentForExperimentStartedAndEndedInPast()
    {
        ...
        Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),-5,-1).setApplication(singleAssignmentContextTestApp);
        exp = postExperiment(exp);
    ...
    public void createAssignmentForExperimentStartedAndEndedInFuture()
    {
        ...
        Experiment exp = ExperimentFactory.createExperiment("_"+UUID.randomUUID().toString(),5,10).setApplication(singleAssignmentContextTestApp);
        exp = postExperiment(exp);
