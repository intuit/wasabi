...
public class EventsIntegrationTest extends TestBase {
    ...
    public void t_PostImpressionWithoutName() {
        User user = userBill;
    ...
    @Test(dependsOnMethods = {"t_PostAssignments"})
    public void t_PostImpressionWithPayload() {
        User user = userBill;
        Event event = EventFactory.createEvent();
    ...
    @Test(dependsOnMethods = {"t_PostAssignments"})
    public void t_PostImpressionAndLoveItWithPayload() {
        User user = userBill;
        ...
        postEvents(events, experiment, user, HttpStatus.SC_CREATED);
    }

    @Test(dependsOnMethods = {"t_PostAssignments"})
    public void t_PostImpressionWithBadDate() {
        User user = userBill;
        Event event = EventFactory.createEvent();
        event.name = null;
        event.context = QBO;
        event.timestamp = "garbage";
        postEvent(event, experiment, user, HttpStatus.SC_BAD_REQUEST);
    }
    ...
    @Test(dependsOnMethods = {"t_PostImpressionAndLoveItWithPayload", "t_PostImpressionWithBadDate",
            "t_PostClickWithPayload", "t_PostClickWithoutName", "t_PostImpressionWithoutName",
            "t_PostClickWithJSONPayloadBad", "t_PostAssignments"})
