    ...
    private void fillLists() {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            AuditLogEntry ale = new AuditLogEntry(time, user, AuditLogAction.UNSPECIFIED_ACTION,
                    exp, Objects.isNull(exp) ? null : Bucket.Label.valueOf("label"),
                    property, Objects.isNull(property) ? null : String.valueOf(i), Objects.isNull(property) ? null : String.valueOf(Math.abs(6 - i)));
            completeList.add(ale);
        }

        appList.addAll(completeList.stream().filter(auditLogEntry -> Objects.nonNull(auditLogEntry.getApplicationName())).collect(Collectors.toList()));
    }
