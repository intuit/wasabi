...
public abstract class PaginationComparator<T> implements Comparator<T> {
    ...
    protected <V extends Enum<V> & PaginationComparatorProperty> int compare(T left, T right, Class<V> enumType) {
        for (String sort : sortOrder.toLowerCase().split(",")) {
            ...
            V property;
            try {
                property = Enum.valueOf(enumType, propertyKey.toUpperCase());
     ...
     * @return -1, 0, 1
     */
    <V> int compareByProperty(T left, T right, Function<T, V> propertyExtractor, BiFunction<V, V, Integer> comparisonFunc, boolean descending) {
        V property1 = null;
        V property2 = null;
        ...
        try {
            ...
            LOG.trace("Can not apply extractor {} on {} -- threw NPE: {}.", propertyExtractor.getClass(), left, ignored.getMessage());
        }
        try {
            property2 = propertyExtractor.apply(right);
        } catch (NullPointerException ignored) {
            LOG.trace("Can not apply extractor {} on {} -- threw NPE: {}.", propertyExtractor.getClass(), right, ignored.getMessage());
        }
