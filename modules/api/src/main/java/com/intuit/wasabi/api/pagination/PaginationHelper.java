...
public class PaginationHelper<T> {
     ...
     */
    public Map<String, Object> paginate(String jsonKey, Collection<T> collection, String filter, String timezoneOffset, String sort, int page, int perPage) {
        Map<String, Object> response = new HashMap<>();

        paginationFilter.replaceFilter(filter, timezoneOffset);
        paginationComparator.replaceSortorder(sort);
        List<T> paginatedList = collection.isEmpty() ? Collections.emptyList() :
                collection.parallelStream()
                        .filter(paginationFilter)
                        .sorted(paginationComparator)
                        .collect(Collectors.toList());
        int totalEntries = paginatedList.size();
        response.put("totalEntries", totalEntries);

        response.put(jsonKey, (perPage == -1 || paginatedList.isEmpty()) ? paginatedList :
                paginatedList.subList(fromIndex(page, perPage, totalEntries),
                        toIndex(page, perPage, totalEntries)));
